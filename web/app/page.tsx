...
function Landing() {
  ...
  const rows = [
    ["Cours TSLA", price > 0 ? usd2(price) : "…"],
    ["Seuil dynamique · Stylus", thresholdBps > 0 ? `${(thresholdBps / 100).toFixed(1)} %` : "…"],
    ["Liquidité du pool", poolLiquidity > 0 ? `${groupInt(poolLiquidity)} USDG` : "…"],
    ["Réseau", "Robinhood Chain · 46630"],
  ];
        ...
        <Reveal delay={0.12}>
          <div className="rounded-2xl border border-hairline bg-paper p-6 shadow-[0_30px_60px_-40px_rgba(28,24,20,0.4)]">
            ...
            <div className="mt-2 flex items-center gap-1.5 text-[11px] text-muted-foreground">
              <Activity className="h-3 w-3 text-ward" /> Valeurs lues sur les contrats déployés.
            </div>
          </div>
        </Reveal>
      </section>
    ...
    <div className="mx-auto max-w-6xl px-6 py-10">
      <Reveal>
        <div className="rounded-2xl border border-hairline bg-paper p-7">
          <div className="font-mono text-[11px] uppercase tracking-wider text-muted-foreground">Solde net</div>
          ...
          <div className="mt-3 inline-flex items-center gap-1.5 rounded-full bg-ward/10 px-2.5 py-1 text-xs font-medium text-ward">
            ...
            {armed ? "Couvert par Ward" : "Protection inactive"}
          </div>
        </div>
      </Reveal>
      ...
      <Reveal delay={0.06}>
        <div className="mt-5 grid grid-cols-2 gap-4 sm:grid-cols-4">
          ...
          <div className="rounded-xl border border-hairline bg-paper p-4">
            ...
            <div className="text-[10px] text-muted-foreground">{hasPosition ? hfLabel(healthFactor) : "aucune dette"}</div>
          </div>
        </div>
      </Reveal>
      ...
      <Reveal delay={0.1}>
        <div className="mt-10 flex items-center justify-between">
          <h2 className="font-serif text-2xl font-semibold tracking-tight">Ton crédit</h2>
        ...
        {hasPosition ? (
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2">
                ...
                {armed ? (
                  ...
                  <span className="rounded-full bg-secondary px-2 py-0.5 font-mono text-[10px] font-medium uppercase tracking-wider text-muted-foreground">Sans Ward</span>
                )}
              </div>
              <div className="mt-0.5 font-mono text-xs text-muted-foreground tnum">{collateral} TSLA · dette {groupInt(debt)} USDG</div>
          ...
          <div className="mt-4 rounded-lg border border-dashed border-hairline bg-paper/50 px-6 py-10 text-center">
            <p className="text-sm text-muted-foreground">Aucun crédit ouvert. Dépose du TSLA et emprunte de l&apos;USDG pour commencer.</p>
            <Link href="/trading" className="mt-4 inline-flex items-center gap-1.5 rounded-md bg-foreground px-4 py-2 text-sm font-medium text-background transition-all hover:brightness-110"><Plus className="h-4 w-4" /> Ouvrir un crédit</Link>
          </div>
        )}
      </Reveal>
      ...
      <Reveal delay={0.14}>
        <Link href="/ward" className="group mt-6 flex items-center gap-4 rounded-xl border border-hairline bg-foreground px-6 py-5 text-background transition-all hover:brightness-110">
          ...
          <div className="flex-1">
            <div className="font-serif text-lg font-semibold">Arme Ward et teste l&apos;anti-liquidation</div>
            <div className="text-sm text-background/70">Baisse le prix et regarde l&apos;autopilote protéger ta position.</div>
          </div>
